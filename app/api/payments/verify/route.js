...
export async function POST(req) {
  ...
  try {
    ...
    let added = 0;
    if (upd.affectedRows === 1) { await addCredits(u.id, p.credits, 'purchase', 'purchase', p.id); added = p.credits; recordEvent({ kind: 'purchase', req, userId: u.id }).catch(() => {}); sendMail({ to: u.email, subject: 'Your CHATWITHPDFAI receipt', text: `Receipt — ${p.credits} credits for INR ${p.amount_inr}. Payment ${paymentId}.`, html: `<p>Thanks for your purchase.</p><p style="font-size:16px"><b>${p.credits} credits</b> &mdash; <b>&#8377;${p.amount_inr}</b></p><p style="color:#666;font-size:13px">Payment ID: ${paymentId}<br/>Order ID: ${orderId}</p><p style="color:#888;font-size:12px">CHATWITHPDFAI &middot; credits never expire</p>` }).catch((e) => console.error('[receipt] mail failed', e.message)); }
