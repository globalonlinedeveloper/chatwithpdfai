...
// Multipart PDF upload -> validate -> extract text -> embed -> persist.
// Gated behind the PRODUCT_MVP feature flag.
//
// Hard limits (anti-abuse / margin protection): 50 MB, 500 pages.
// Heavy work runs through the serialized queue (concurrency 1).
//
// ORDER MATTERS: we parse the PDF BEFORE any database work. This (a) avoids
// creating a document row for a PDF that turns out to be unparseable, and
// (b) keeps PDF parsing off the same tick as a live mysql2 query — pdf-parse's
// bundled pdf.js proved fragile when a DB query ran immediately before it.
// Until auth (M5) the owner is a stub user id behind the flag.
// =================================================================
...
const MAX_PAGES = 500;
const STUB_USER_ID = Number(process.env.STUB_USER_ID || 1); // replaced by session in M5
...
async function uploadDirFor(userId) {
  if (isTestMode()) {
    return path.join(os.tmpdir(), 'cwpai-test-uploads', String(userId));
  }
  const base =
    process.env.UPLOAD_DIR ||
    path.join(os.homedir(), 'domains', 'chatwithpdfai.com', 'uploads');
...
export async function POST(req) {
  ...
  // ---------- cheap validation before reading bytes ----------
  ...
  // ---------- extract -> persist -> embed (serialized) ----------
  // Extraction runs first (no DB yet). DB work happens only after a clean parse.
  let documentId;
  try {
    const result = await enqueue(async () => {
      // 1) parse the PDF (no database activity before this point)
      ...
      // 3) create the document row (first DB touch)
      ...
      const pageRows = embeddable.map((p, i) => ({
        ...
        chunk_index: 0,
        text: p.text,
        token_count: p.tokenCount || 0,
        source: p.needsOcr ? 'ocr' : 'text',
    ...
    return NextResponse.json({
      ...
      document: {
        ...
        embeddedPages: result.embeddedPages,
    ...
    if (documentId != null) {
      await documents
        .updateDocument(documentId, { status: 'failed', error_message: String(e.message || e).slice(0, 500) })
        .catch(() => {});
    }
    if (status >= 500) console.error('[upload] processing failed', e);
    const msg =
      status === 413 ? e.message : status === 422 ? 'Could not read this PDF (it may be corrupt or image-only).' : 'Failed to process the PDF';
