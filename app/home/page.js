...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/me').then((r) => (r.ok ? r.json() : null)).then((j) => { if (j && j.user) setName(String(j.user.name || j.user.email || '').split('@')[0]); }).catch(() => {});
    fetch('/api/documents').then((r) => (r.ok ? r.json() : null)).then((j) => { if (j && Array.isArray(j.documents)) setDocs(j.documents); }).catch(() => {});
    fetch('/api/studio/papers').then((r) => (r.ok ? r.json() : null)).then((j) => { if (j && Array.isArray(j.papers)) setPapers(j.papers); }).catch(() => {});
  }, []);
  const readyDocs = docs.filter((d) => d.status === 'ready');
  const recent = [
    ...papers.map((p) => ({ key: 'p' + p.id, kind: 'paper', title: p.title, at: p.createdAt, href: '/studio' })),
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: 860, margin: '0 auto', width: '100%', padding: '30px 20px 60px' }}>
        {credits != null && credits < 10 && <div style={{ background: 'rgba(255,189,46,0.12)', border: '1px solid rgba(255,189,46,0.4)', borderRadius: 'var(--r)', padding: '10px 14px', marginBottom: 16, fontSize: 13.5, color: '#ffd27a', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><span style={{ flex: 1, minWidth: 200 }}>You’re low on credits ({credits} left).</span><a href="/buy" className="btn btn-glass btn-sm">Buy credits</a></div>}
