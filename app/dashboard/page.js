...
function Tile({ icon, title, desc, openHref, openLabel, newHref, newLabel, count, loaded }) {
  const empty = loaded && !count;
  return (
    <div className="glass" style={{ padding: '20px 22px', borderRadius: 'var(--r-xl)', border: '1px solid var(--stroke-2)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 16, minHeight: 32 }}>
        {!loaded ? null : empty ? (
          <a href={newHref} className="btn btn-iris btn-sm">{newLabel}</a>
        ) : (
          <>
...
export default function HomePage() {
  ...
  const recent = [
    ...papers.map((p) => ({ key: 'p' + p.id, kind: 'paper', label: 'Paper', tint: 'var(--violet-2)', icon: PAPER_S, title: p.title, at: p.createdAt, href: '/question-paper-generator?paper=' + p.id, meta: p.numQuestions ? p.numQuestions + ' Qs' : '' })),
    ...readyDocs.map((d) => ({ key: 'd' + d.id, kind: 'document', label: 'PDF', tint: 'var(--blue)', icon: DOC_S, title: d.filename, at: d.createdAt || d.uploadedAt, href: '/chat-with-pdf?doc=' + d.id, meta: [d.pageCount ? d.pageCount + ' pp' : '', fmtSize(d.sizeBytes)].filter(Boolean).join(' · ') })),
    ...tests.map((t) => ({ key: 't' + t.id, kind: 'test', label: 'Test', tint: 'var(--green)', icon: TEST_S, title: t.title, at: t.createdAt, href: '/t/' + t.token, external: true, token: t.token, meta: t.attempts ? t.attempts + ' attempt' + (t.attempts > 1 ? 's' : '') : 'no attempts yet' })),
  ...
  let suggest = null;
  if (loaded) {
    const totalAttempts = tests.reduce((n, t) => n + (t.attempts || 0), 0);
    ...
    else if (totalAttempts > 0) suggest = { text: totalAttempts + ' student attempt' + (totalAttempts > 1 ? 's' : '') + ' submitted — review the results.', href: '/library?tab=tests', cta: 'Review' };
  }
  const tileMeta = {
    'chat-with-pdf': { openLabel: readyDocs.length ? ('Open \u00b7 ' + readyDocs.length + ' doc' + (readyDocs.length > 1 ? 's' : '')) : 'Open', newHref: '/chat-with-pdf', newLabel: '+ Upload PDF', count: readyDocs.length },
    'question-paper-generator': { openLabel: papers.length ? ('Open \u00b7 ' + papers.length + ' paper' + (papers.length > 1 ? 's' : '')) : 'Open', newHref: '/question-paper-generator', newLabel: '+ New paper', count: papers.length },
  };
  const stats = [
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <AppNav active="dashboard" credits={credits} />
      <main id="main" style={{ maxWidth: 980, margin: '0 auto', width: '100%', padding: '30px 20px 60px' }}>
        ...
        <div style={{ fontSize: 13.5, color: 'var(--text-3)', marginBottom: 18 }}>Pick a tool to get started.</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14, marginBottom: suggest ? 14 : 30 }}>
          {liveTools().map((t) => { const m = tileMeta[t.slug] || {}; return <Tile key={t.slug} icon={<ToolIcon name={t.icon} size={22} stroke="#fff" />} title={t.name} desc={t.tagline} openHref={t.appHref} openLabel={m.openLabel || 'Open'} newHref={m.newHref || t.appHref} newLabel={m.newLabel || 'Open'} count={m.count || 0} loaded={loaded} />; })}
        </div>
        ...
        {recent.length > 0 && (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {recent.map((r) => (
                <div key={r.key} className="glass" style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '8px 12px', borderRadius: 'var(--r)' }}>
                  <span style={{ width: 22, height: 22, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: r.tint }}>{r.icon}</span>
                  <a href={r.href} {...(r.external ? { target: '_blank', rel: 'noreferrer' } : {})} style={{ flex: 1, minWidth: 0, textDecoration: 'none', color: 'var(--text)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: r.tint, border: '1px solid var(--stroke-2)', borderRadius: 5, padding: '1px 6px', flexShrink: 0 }}>{r.label}</span>
                      <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13.5 }}>{r.title}</span>
                    </div>
                    <div className="mono" style={{ fontSize: 10.5, color: 'var(--text-4)', marginTop: 3 }}>{r.meta ? r.meta : ''}{r.meta && r.at ? ' · ' : ''}{r.at ? relTime(r.at) : ''}</div>
                  </a>
