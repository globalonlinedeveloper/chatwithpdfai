...
import { TYPE_LABELS, ALL_TYPES, LANG_NAME, isAuto, clampInt, clampHalf, PaperView, PromptStem, PracticeInput, Feedback, EditAnswerControl } from './paper-render';

// OMR sheet is lazy-loaded — only needed when the user toggles it, so it stays out of the initial bundle.
const OMRSheet = dynamic(() => import('./paper-render-omr').then((m) => m.OMRSheet), { ssr: false, loading: () => <div style={{ padding: 20, textAlign: 'center', color: '#888', fontSize: 13 }}>Loading OMR sheet…</div> });
...
export default function PapersPage() {
  ...
  const [buildOpen, setBuildOpen] = useState(true); // middle build-form panel collapse toggle
  const [mobileTab, setMobileTab] = useState('build'); // mobile Build/Preview tab
  ...
  const [negMark, setNegMark] = useState(0); // online-test negative marking: marks deducted per wrong answer (0 = off)
  ...
  const [secOpen, setSecOpen] = useState({ lib: true, shares: true, bank: true, templates: true });
  const [cardOpen, setCardOpen] = useState({ struct: true, sections: true, options: true });
  ...
  const [sampleQ, setSampleQ] = useState(null); // 1-question 'preview a sample' result (see quality before spending on the full paper)
  ...
  useEffect(() => { fetch('/api/credits').then((r) => { if (r.status === 401) { window.location.href = '/signin?next=' + encodeURIComponent(window.location.pathname + window.location.search); return null; } return r.json(); }).then((j) => { if (j && typeof j.balance === 'number') setCredits(j.balance); }).catch(() => {});
    loadLibrary(); loadShares(); loadTemplates(); try { const sp = new URLSearchParams(window.location.search); const pid = Number(sp.get('paper')) || 0; if (pid) openPaper(pid); const _test = Number(sp.get('test')) || 0; if (_test) viewAttempts(_test); const tid = Number(sp.get('tpl')) || 0; if (tid) fetch('/api/papers/templates').then((r) => r.ok ? r.json() : null).then((j) => { const list = j && Array.isArray(j.templates) ? j.templates : []; const t = list.find((x) => Number(x.id) === tid); if (t) applyTemplate(t); }).catch(() => {}); } catch (e) {} }, []);
  ...
  useEffect(() => { if (!savedMsg) return; const t = setTimeout(() => setSavedMsg(''), 4000); return () => clearTimeout(t); }, [savedMsg]);
  useEffect(() => { if (!uploadMsg || uploading) return; const t = setTimeout(() => setUploadMsg(''), 6000); return () => clearTimeout(t); }, [uploadMsg, uploading]);
  ...
  function moveSec(i, dir) { setSections((cur) => { const j = i + dir; if (j < 0 || j >= cur.length) return cur; const a = cur.slice(); const t = a[i]; a[i] = a[j]; a[j] = t; return a; }); }
  function dupSec(i) { setSections((cur) => { if (cur.length >= 8) return cur; const a = cur.slice(); a.splice(i + 1, 0, { ...cur[i] }); return a; }); }
  function applyStarter(secs) { setBpKey('custom'); setExamStyle(''); setBpTopic(''); setFullSize(false); setFullConfirm(false); setSections(secs.map((x) => ({ ...x }))); setBuildOpen(true); setTimeout(() => { try { const el = document.querySelector('[data-testid="topic"]'); if (el) { el.scrollIntoView({ block: 'center', behavior: 'smooth' }); el.focus(); } } catch (e) {} }, 60); }
  function loadTemplates() { fetch('/api/papers/templates').then((r) => r.ok ? r.json() : null).then((j) => { if (j && Array.isArray(j.templates)) setTemplates(j.templates); }).catch(() => {}); }
  function tplSummary(tpl) { const c = (tpl && tpl.config) || {}; const secs = Array.isArray(c.sections) ? c.sections : []; const n = secs.length; const marks = secs.reduce((m, x) => m + (Number(x.count) || 0) * (Number(x.marks) || 0), 0); const types = [...new Set(secs.map((x) => x.type).filter(Boolean))].slice(0, 3).join(', '); return n + ' section' + (n === 1 ? '' : 's') + (marks ? ' · ' + marks + ' marks' : '') + (types ? ' · ' + types : ''); }
  function startSaveTemplate() { setTplName((bpKey !== 'custom' && examStyle) || topic || (sections[0] && sections[0].title) || 'My template'); setSecOpen((x) => ({ ...x, templates: true })); setSavingTpl(true); }
  function commitTemplate() { const name = String(tplName || '').trim(); if (!name) return; const config = { topic, bpTopic, examStyle, bpKey, sections: sections.map((x) => ({ ...x })), difficulty, cognitive, language, sets, layout, negMark, watermark, includeKey, verify, institution, instructions }; fetch('/api/papers/templates', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.slice(0, 120), config }) }).then((r) => r.json().catch(() => ({}))).then((j) => { if (j && j.ok) { setSavingTpl(false); setTplName(''); setSavedMsg('Template saved'); loadTemplates(); setTimeout(() => setSavedMsg(''), 1800); } else setSavedMsg((j && j.error) || 'Could not save template'); }).catch(() => {}); }
  function applyTemplate(tpl) { const c = (tpl && tpl.config) || {}; setBpKey(c.bpKey || 'custom'); setExamStyle(c.examStyle || ''); setBpTopic(c.bpTopic || ''); setTopic(c.topic || ''); setFullSize(false); setFullConfirm(false); if (Array.isArray(c.sections) && c.sections.length) setSections(c.sections.map((x) => ({ ...x }))); if (c.difficulty) setDifficulty(c.difficulty); setCognitive(c.cognitive || ''); if (c.language) setLanguage(c.language); if (c.sets != null) setSets(c.sets); if (c.layout) setLayout(c.layout); if (c.negMark != null) setNegMark(c.negMark); if (typeof c.watermark === 'boolean') setWatermark(c.watermark); if (typeof c.includeKey === 'boolean') setIncludeKey(c.includeKey); if (typeof c.verify === 'boolean') setVerify(c.verify); if (c.institution != null) setInstitution(c.institution); if (c.instructions != null) setInstructions(c.instructions); setBuildOpen(true); }
  function delTemplate(id) { setTemplates((cur) => cur.filter((t) => t.id !== id)); fetch('/api/papers/templates?id=' + id, { method: 'DELETE' }).catch(() => {}); }
  ...
  async function exportScores(title) { const rows = [['Student', 'Score', 'Total', 'Percent', 'Submitted']]; (attemptList || []).forEach((a) => rows.push([a.name || 'Anonymous', a.score, a.total, a.total ? Math.round(100 * a.score / a.total) + '%' : '', a.createdAt ? new Date(a.createdAt).toLocaleString() : ''])); const csv = rows.map((r) => r.map((c) => '"' + String(c == null ? '' : c).replace(/"/g, '""') + '"').join(',')).join('\r\n'); try { const m = await import('./exporters'); m.downloadText(m.slug(title || 'scores') + '-scores.csv', csv, 'text/csv'); } catch (e) {} }
  ...
  function moveQuestion(gi, dir) { if (!paper) return; pushHist(); setPaper((pp) => { if (!pp) return pp; let base = 0; const sections = pp.sections.map((sec) => { const qs = sec.questions || []; const pos = gi - base; const inThis = pos >= 0 && pos < qs.length; base += qs.length; if (!inThis) return sec; const np = pos + dir; if (np < 0 || np >= qs.length) return sec; const a = qs.slice(); const t = a[pos]; a[pos] = a[np]; a[np] = t; return { ...sec, questions: a }; }); return { ...pp, sections }; }); }
  function addHotspot() { if (!paper) return; pushHist(); setPaper((pp) => { if (!pp || !pp.sections.length) return pp; const sections = pp.sections.map((sec, i) => i === pp.sections.length - 1 ? { ...sec, questions: [...sec.questions, { type: 'hotspot', q: 'Click the correct location on the image', image: '', hot: null }] } : sec); const totalMarks = sections.reduce((t, sec) => t + sec.questions.length * (Number(sec.marks) || 1), 0); return { ...pp, sections, totalMarks }; }); }
  ...
  function onQImageFile(file, gi) { if (!file) return; const reader = new FileReader(); reader.onload = () => { const im = new Image(); im.onload = () => { try { const max = 480; const sc = Math.min(1, max / (im.width || 1)); const w = Math.max(1, Math.round((im.width || 1) * sc)); const h = Math.max(1, Math.round((im.height || 1) * sc)); const cv = document.createElement('canvas'); cv.width = w; cv.height = h; cv.getContext('2d').drawImage(im, 0, 0, w, h); let url = cv.toDataURL('image/jpeg', 0.82); if (url.length > 600000) url = cv.toDataURL('image/jpeg', 0.6); if (url.length > 950000) { setNote('That image is too large even after resizing.'); return; } editPatch(gi, { image: url }); } catch (e) { setNote('Could not process that image.'); } }; im.onerror = () => setNote('Could not read that image.'); im.src = String(reader.result); }; reader.readAsDataURL(file); }
  ...
  async function regenSection(si) {
    ...
    try {
      const body = { topic: (topic.trim() || paper.title || ''), examStyle: effExamStyle, level, difficulty, language, institution, instructions, sections: [{ title: sec.title || '', types: (sec.types && sec.types.length ? sec.types : ['mcq']), count: sec.questions.length || 5, marks: Number(sec.marks || 1), difficulty: sec.difficulty || '', cognitive: sec.cognitive || '' }], nonce: Math.random().toString(36).slice(2), exclude: exclude.slice(-80), verify: false, documentId: sourceDocId };
      const r = await fetch('/api/papers/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const j = await r.json().catch(() => ({}));
  ...
  async function runExport(kind, p, label) {
    try {
      const m = await import('./exporters');
      const base = m.slug(p.title) + (label ? '-' + label : '');
      if (kind === 'xml') m.downloadText(base + '.xml', m.toMoodleXML(p), 'application/xml');
      else if (kind === 'gift') m.downloadText(base + '.gift.txt', m.toGIFT(p));
      else if (kind === 'aiken') m.downloadText(base + '.aiken.txt', m.toAiken(p));
      else if (kind === 'qti') m.downloadBlob(base + '.qti.zip', m.toQTIZip(p));
      else if (kind === 'csv') m.downloadText(base + '.csv', m.toCSV(p), 'text/csv');
    } catch (e) { setNote('Export failed \u2014 please retry.'); }
  }
  ...
  async function previewSample() {
    ...
    if (eff.length < 3 && !sourceDocId) { setNote('Describe a topic, pick a blueprint, or attach a PDF.'); return; }
    if (typeof credits === 'number' && credits < 1) { setNote("You're out of credits \u2014 buy a pack to generate."); return; }
    const sec = sections[0] || { title: '', type: 'mcq', marks: 1 };
    setSampleBusy(true); setNote('');
    try {
      const r = await fetch('/api/papers/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic: eff, examStyle: effExamStyle, level, difficulty, cognitive, language, institution, instructions, sections: [{ title: sec.title, types: [sec.type], count: 1, marks: Number(sec.marks || 1) }], nonce: Math.random().toString(36).slice(2), verify: false, documentId: sourceDocId }) });
      const j = await r.json().catch(() => ({}));
      if (!r.ok) {
        if (r.status === 401) { window.location.href = '/signin?next=' + encodeURIComponent(window.location.pathname + window.location.search); return; }
        if (r.status === 403) setNote('Please verify your email before generating.');
        else if (r.status === 402) setNote('You are out of credits.');
        else setNote(j.error || 'Could not generate a sample.');
        return;
      }
      const q = j.paper && Array.isArray(j.paper.sections) && j.paper.sections[0] && j.paper.sections[0].questions && j.paper.sections[0].questions[0];
      if (q) setSampleQ({ q, marks: Number(sec.marks || 1) }); else setNote('Could not generate a sample \u2014 try again.');
      if (typeof j.balance === 'number') setCredits(j.balance);
    } catch (e) { setNote((e && e.message) || 'Could not generate a sample.'); }
  ...
  const tplShown = tplQ.trim() ? templates.filter((tp) => (tp.name || '').toLowerCase().includes(tplQ.trim().toLowerCase())) : templates;
    ...
    <div id="papers-shell" style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div className="papers-body" style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        <aside className="no-print papers-aside" style={{ width: 248, flexShrink: 0, borderRight: '1px solid var(--stroke-1)', background: 'rgba(5,6,20,0.6)', backdropFilter: 'blur(20px) saturate(180%)', display: asideOpen ? 'flex' : 'none', flexDirection: 'column', overflowY: 'auto', padding: '14px 12px' }}>
          <button type="button" onClick={() => setAsideOpen(false)} aria-label="Hide papers panel" title="Hide panel" data-testid="aside-hide" className="btn btn-glass btn-sm" style={{ alignSelf: 'flex-end', padding: '2px 9px', marginBottom: 8 }}>« Hide papers</button>
          ...
          <div onClick={() => setSecOpen((x) => ({ ...x, lib: !x.lib }))} role="button" tabIndex={0} data-testid="sec-lib" className="eyebrow" style={{ marginBottom: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ fontSize: 9, width: 8, display: 'inline-block' }}>{secOpen.lib ? '▾' : '▸'}</span>Question papers{library.length ? ' (' + library.length + ')' : ''}</div>{secOpen.lib && (<>
          {library.length === 0 ? <div style={{ fontSize: 11.5, color: 'var(--text-4)', marginBottom: 18 }}>Saved papers appear here.</div> : (<>
            <input className="input" value={libQ} onChange={(e) => setLibQ(e.target.value)} placeholder="Search saved papers…" aria-label="Search question papers" data-testid="lib-search" style={{ width: '100%', boxSizing: 'border-box', fontSize: 11.5, padding: '6px 9px', marginBottom: 6 }} />
        ...
        </>)}<div style={{ marginTop: 0, marginBottom: 8 }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginBottom: 8 }}><div onClick={() => setSecOpen((x) => ({ ...x, templates: !x.templates }))} role="button" tabIndex={0} data-testid="sec-templates" className="eyebrow" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, marginBottom: 0 }}><span style={{ fontSize: 9, width: 8, display: 'inline-block' }}>{secOpen.templates ? '▾' : '▸'}</span>Templates{templates.length ? ' (' + templates.length + ')' : ''}</div><button type="button" onClick={startSaveTemplate} className="btn btn-glass btn-sm" data-testid="save-template" style={{ fontSize: 10.5, padding: '3px 8px' }} title="Save the current build setup (topic, blueprint, all sections, options) as a reusable template">+ Save current</button></div>{secOpen.templates && <>{savingTpl && <div data-testid="tpl-save-form" style={{ background: 'var(--glass-2)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '8px 9px', marginBottom: 8 }}><div style={{ fontSize: 10.5, color: 'var(--text-4)', marginBottom: 5 }}>Name this template</div><input autoFocus value={tplName} onChange={(e) => setTplName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); commitTemplate(); } else if (e.key === 'Escape') setSavingTpl(false); }} aria-label="Template name" data-testid="tpl-name" className="input" maxLength={120} style={{ width: '100%', boxSizing: 'border-box', fontSize: 12.5, padding: '6px 9px', marginBottom: 7 }} /><div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><button type="button" onClick={commitTemplate} disabled={!tplName.trim()} className="btn btn-iris btn-sm" data-testid="tpl-save-confirm" style={{ fontSize: 11, padding: '4px 11px' }}>Save</button><button type="button" onClick={() => setSavingTpl(false)} className="btn btn-glass btn-sm" data-testid="tpl-save-cancel" style={{ fontSize: 11, padding: '4px 11px' }}>Cancel</button><span style={{ flex: 1 }} /><span style={{ fontSize: 9.5, color: 'var(--text-4)' }}>Enter · Esc</span></div></div>}{(templates.length === 0 ? (savingTpl ? null : <div style={{ fontSize: 11.5, color: 'var(--text-4)' }}>Save a build setup to reuse it across your devices.</div>) : <><input className="input" value={tplQ} onChange={(e) => setTplQ(e.target.value)} placeholder="Search templates…" aria-label="Search templates" data-testid="tpl-search" style={{ width: '100%', boxSizing: 'border-box', fontSize: 11.5, padding: '6px 9px', marginBottom: 6 }} />{tplShown.length === 0 ? <div style={{ fontSize: 11.5, color: 'var(--text-4)' }}>No templates match.</div> : <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }} data-testid="my-templates">{tplShown.map((tpl) => <div key={tpl.id} className="glass" style={{ padding: '7px 9px', borderRadius: 'var(--r)' }} data-testid="tpl-row"><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><button type="button" onClick={() => applyTemplate(tpl)} data-testid="tpl-apply" title="Load this setup, then Generate" style={{ flex: 1, minWidth: 0, textAlign: 'left', border: 'none', background: 'transparent', color: 'var(--text-2)', cursor: 'pointer', fontSize: 12.5, padding: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tpl.name}</button><button type="button" onClick={() => delTemplate(tpl.id)} aria-label={'Delete template ' + tpl.name} data-testid="tpl-del" className="btn btn-glass btn-sm" style={{ fontSize: 10.5, padding: '2px 7px' }}>{'✕'}</button></div><div className="mono" style={{ fontSize: 9.5, color: 'var(--text-4)', marginTop: 3 }}>{tplSummary(tpl)}</div></div>)}</div>}</>)}</>}</div></aside>

        <div id="main" className="papers-main" data-mtab={mobileTab} style={{ display: 'flex', flex: 1, minWidth: 0 }}><div className="qpg-tabs no-print" data-testid="mobile-tabs" style={{ gap: 6, padding: '8px 12px', borderBottom: '1px solid var(--stroke-1)', background: 'rgba(5,6,20,0.4)' }}><button type="button" onClick={() => setMobileTab('build')} className={mobileTab === 'build' ? 'btn btn-iris btn-sm' : 'btn btn-glass btn-sm'} data-testid="mtab-build" aria-pressed={mobileTab === 'build'} style={{ flex: 1 }}>Build</button><button type="button" onClick={() => setMobileTab('preview')} className={mobileTab === 'preview' ? 'btn btn-iris btn-sm' : 'btn btn-glass btn-sm'} data-testid="mtab-preview" aria-pressed={mobileTab === 'preview'} style={{ flex: 1 }}>Preview</button></div>
          <h1 className="sr-only">Question paper generator</h1>
          <section className="no-print papers-build" style={{ display: buildOpen ? 'block' : 'none', width: asideOpen ? 472 : 660, flexShrink: 0, borderRight: '1px solid var(--stroke-1)', overflowY: 'auto', padding: '18px 20px', background: 'rgba(5,6,20,0.35)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 12 }}>{!asideOpen ? <button type="button" onClick={() => setAsideOpen(true)} aria-label="Show papers panel" title="Show library & papers" data-testid="aside-show" className="btn btn-glass btn-sm" style={{ padding: '3px 10px' }}>» Library &amp; papers</button> : <span />}<button type="button" onClick={() => setBuildOpen(false)} aria-label="Hide build panel" title="Hide build panel" data-testid="build-hide" className="btn btn-glass btn-sm" style={{ padding: '2px 9px' }}>« Hide</button></div>
            <div data-testid="build-steps" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, marginBottom: 14, flexWrap: 'wrap' }}>{['1 Blueprint', '2 Source', '3 Sections', 'Generate'].map((st, i) => { const okSetup = canGen || Boolean(paper); const complete = [okSetup, okSetup, (okSetup && ((isFullRO ? fullTotalQ : totalQ) > 0)), Boolean(paper)][i]; const active = (canGen ? 3 : 0) === i; return <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>{i ? <span style={{ color: 'var(--text-4)' }}>›</span> : null}<span data-step-active={active ? '1' : undefined} style={{ padding: '2px 8px', fontSize: 10.5, background: active ? 'var(--violet)' : 'var(--glass-2)', borderRadius: 999, color: active ? '#fff' : (complete ? 'var(--violet-2)' : 'var(--text-4)'), border: '1px solid ' + (active ? 'var(--violet)' : 'transparent') }}>{complete ? '✓ ' : ''}{st}</span></span>; })}</div>
            <button type="button" onClick={() => setCardOpen((o) => ({ ...o, struct: !o.struct }))} aria-expanded={cardOpen.struct} data-testid="card-struct" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: 'var(--glass-2)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '8px 12px', margin: '4px 0 ' + (cardOpen.struct ? '12px' : '4px'), color: 'var(--text-2)', fontSize: 11.5, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit' }}><span>Structure &amp; source</span><span style={{ fontSize: 10, color: 'var(--text-3)' }}>{cardOpen.struct ? '▾' : '▸'}</span></button>
            {cardOpen.struct && (<>
            ...
            <div ref={srcBoxRef} style={{ position: 'relative' }}>
              ...
              <input ref={fileRef} type="file" accept="application/pdf,.pdf" aria-label="Upload a PDF" onChange={(e) => { const f = e.target.files && e.target.files[0]; e.target.value = ''; if (f) uploadSource(f); }} style={{ display: 'none' }} data-testid="source-file" aria-hidden="true" tabIndex={-1} />
            </div>
            {uploadMsg ? <div data-testid="upload-msg" role="status" aria-live="polite" style={{ fontSize: 11.5, color: uploading ? 'var(--text-3)' : 'var(--text-2)', marginTop: 6 }}>{uploadMsg}</div> : null}
            ...
            <button type="button" onClick={() => setCardOpen((o) => ({ ...o, sections: !o.sections }))} aria-expanded={cardOpen.sections} data-testid="card-sections" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: 'var(--glass-2)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '8px 12px', margin: '4px 0 ' + (cardOpen.sections ? '12px' : '4px'), color: 'var(--text-2)', fontSize: 11.5, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit' }}><span>Sections</span><span style={{ fontSize: 10, color: 'var(--text-3)' }}>{cardOpen.sections ? '▾' : '▸'}</span></button>
            {cardOpen.sections && (<>
            ...
            {isFullRO ? (
              ...
              {sections.map((s, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 7, padding: '8px 9px', background: 'var(--glass-1)', border: '1px solid var(--stroke-1)', borderRadius: 'var(--r)' }} data-testid="section-row">
                  ...
                  <select value={s.type} onChange={(e) => setSec(i, { type: e.target.value })} aria-label="Question type" style={{ ...ctrl, width: '100%', boxSizing: 'border-box' }}>{ALL_TYPES.map((t) => <option key={t} value={t}>{TYPE_LABELS[t]}</option>)}</select>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                    <label style={{ fontSize: 11.5, color: 'var(--text-3)' }}>Qs <input type="number" min={1} max={30} value={s.count} onChange={(e) => setSec(i, { count: clampInt(e.target.value, 1, 30) })} aria-label="Questions in section" className="qpg-num" style={{ ...ctrl, width: 50 }} /></label>
                    <label style={{ fontSize: 11.5, color: 'var(--text-3)' }}>Marks <input type="number" min={1} max={20} step={0.5} value={s.marks} onChange={(e) => setSec(i, { marks: clampHalf(e.target.value, 1, 20) })} aria-label="Marks per question" className="qpg-num" style={{ ...ctrl, width: 50 }} /></label><label style={{ fontSize: 11.5, color: 'var(--text-3)' }} title="Answer any N of these questions (0 = all compulsory)">Any <input type="number" min={0} max={Number(s.count) || 30} value={s.choose || 0} onChange={(e) => setSec(i, { choose: clampInt(e.target.value, 0, Number(s.count) || 30) })} aria-label="Answer any N (0 = all compulsory)" className="qpg-num" data-testid="sec-choose" style={{ ...ctrl, width: 42 }} /></label><span data-testid="sec-marks" title="Section total = questions × marks" style={{ fontSize: 11, color: 'var(--text-3)', whiteSpace: 'nowrap', minWidth: 34, textAlign: 'right' }}>= {(((Number(s.choose) || 0) > 0 && (Number(s.choose) || 0) < (Number(s.count) || 0)) ? (Number(s.choose) || 0) : (Number(s.count) || 0)) * (Number(s.marks) || 0)}</span>
                    <span style={{ display: 'inline-flex', gap: 5, marginLeft: 'auto', flexShrink: 0 }}><button type="button" onClick={() => moveSec(i, -1)} disabled={i === 0} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Move section up" data-testid={'secup-' + i}>{'↑'}</button><button type="button" onClick={() => moveSec(i, 1)} disabled={i === sections.length - 1} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Move section down" data-testid={'secdown-' + i}>{'↓'}</button><button type="button" onClick={() => dupSec(i)} disabled={sections.length >= 8} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Duplicate section" data-testid={'secdup-' + i} title="Duplicate this section">{'⧉'}</button><button type="button" onClick={() => delSec(i)} className="btn btn-glass btn-sm" style={{ padding: '5px 9px' }} aria-label="Remove section">{'✕'}</button></span>
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', fontSize: 11.5, color: 'var(--text-3)' }}><label style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Difficulty<select value={s.diff || ''} onChange={(e) => setSec(i, { diff: e.target.value })} aria-label="Section difficulty" data-testid={'sec-diff-' + i} style={{ ...ctrl, padding: '4px 6px' }}><option value="">Paper default</option><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option><option value="mixed">Mixed</option></select></label><label style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Focus<select value={s.cog || ''} onChange={(e) => setSec(i, { cog: e.target.value })} aria-label="Section cognitive focus" data-testid={'sec-cog-' + i} style={{ ...ctrl, padding: '4px 6px' }}><option value="">Paper default</option><option value="recall">Recall</option><option value="application">Application</option></select></label></div><input value={s.note || ''} onChange={(e) => setSec(i, { note: e.target.value })} placeholder="Section instructions (optional) — e.g. Answer all questions" aria-label="Section instructions" data-testid="sec-note" className="input" style={{ width: '100%', boxSizing: 'border-box', fontSize: 11.5, padding: '6px 9px' }} />
                </div>
              ))}
            ...
            <button type="button" onClick={() => setCardOpen((o) => ({ ...o, options: !o.options }))} aria-expanded={cardOpen.options} data-testid="card-options" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: 'var(--glass-2)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '8px 12px', margin: '4px 0 ' + (cardOpen.options ? '12px' : '4px'), color: 'var(--text-2)', fontSize: 11.5, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit' }}><span>Options</span><span style={{ fontSize: 10, color: 'var(--text-3)' }}>{cardOpen.options ? '▾' : '▸'}</span></button>
            {cardOpen.options && (<>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginTop: 12 }}>
              ...
              <label style={{ fontSize: 12, color: 'var(--text-2)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }} title="A second AI pass re-checks the answer key"><input type="checkbox" checked={verify} onChange={(e) => setVerify(e.target.checked)} /> Verify answers</label><label style={{ fontSize: 12, color: 'var(--text-2)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }} title="Faint diagonal watermark (institution or title) on the printed paper"><input type="checkbox" checked={watermark} onChange={(e) => setWatermark(e.target.checked)} data-testid="watermark-toggle" /> Watermark</label><label style={{ fontSize: 12, color: 'var(--text-2)', display: 'flex', alignItems: 'center', gap: 6 }} title="Online tests only: marks deducted for each wrong answer (0 = no negative marking)">Neg. mark <input type="number" min={0} max={5} step={0.25} value={negMark} onChange={(e) => setNegMark(Math.max(0, Math.min(5, Number(e.target.value) || 0)))} aria-label="Negative marking per wrong answer" className="qpg-num" data-testid="neg-mark" style={{ ...ctrl, width: 56 }} /></label>
            ...
            {canGen && !isFullRO ? <div data-testid="preflight" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 11, color: 'var(--text-3)', margin: '0 0 8px' }}><span style={{ background: 'var(--glass-1)', border: '1px solid var(--stroke-2)', borderRadius: 999, padding: '2px 9px' }}>{totalQ} questions</span><span style={{ background: 'var(--glass-1)', border: '1px solid var(--stroke-2)', borderRadius: 999, padding: '2px 9px' }}>{dispTotalMarks} marks</span>{sets > 1 ? <span style={{ background: 'var(--glass-1)', border: '1px solid var(--stroke-2)', borderRadius: 999, padding: '2px 9px' }}>{sets} sets</span> : null}</div> : null}
            ...
            <button type="button" onClick={previewSample} disabled={sampleBusy || busy || (!fullSize && !canGen)} className="btn btn-glass btn-sm" data-testid="preview-sample" title="Generate one example question to check style and quality before building the full paper" style={{ width: '100%', marginTop: 6, opacity: (sampleBusy || busy || (!fullSize && !canGen)) ? 0.6 : 1 }}>{sampleBusy ? 'Generating sample\u2026' : '\ud83d\udc41 Preview a sample question'}</button>
            {sampleQ ? (
              <div data-testid="sample-card" style={{ marginTop: 10, border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: 'var(--glass-2)', fontSize: 11, color: 'var(--text-3)' }}><span>Sample preview · 1 question (not charged toward your paper)</span><button type="button" onClick={() => setSampleQ(null)} data-testid="sample-dismiss" className="btn btn-glass btn-sm" style={{ padding: '1px 8px' }}>Dismiss</button></div>
                <div style={{ background: '#fff', color: '#111', padding: '12px 14px', overflowX: 'auto' }}><PaperView paper={{ title: (topic.trim() ? topic.trim().slice(0, 60) : (examStyle || 'Sample')), examStyle, sections: [{ title: '', marks: sampleQ.marks, questions: [sampleQ.q] }] }} layout="clean" includeKey={true} /></div>
              </div>
            ) : null}
            {fullConfirm && !busy ? (
            ...
            {note && <div role="status" aria-live="assertive" style={{ marginTop: 12, fontSize: 13, color: '#ffb4b4' }}>{note} {note.includes('credits') && <a href="/buy" style={{ color: 'var(--violet-2)' }}>Buy credits →</a>}</div>}
            <div className="mono" style={{ marginTop: 12, fontSize: 10, color: 'var(--text-3)', letterSpacing: '0.06em' }}>Answers are AI-generated &mdash; spot-check before using in a real exam.</div>
          </section>

          <section className="papers-preview" style={{ flex: 1, minWidth: 0, overflowY: 'auto', padding: '18px 20px', background: 'rgba(5,6,20,0.5)' }}>
            {!buildOpen && <button type="button" onClick={() => setBuildOpen(true)} aria-label="Show build panel" title="Show build panel" data-testid="build-show" className="btn btn-glass btn-sm no-print" style={{ marginBottom: 12, padding: '3px 10px' }}>» Build a paper</button>}
            {!paper ? (
              <div className="no-print" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: 'var(--text-4)' }}>
                {busy ? (
                  ...
                  <div style={{ maxWidth: 380 }} data-testid="empty-state">
                    ...
                    <div style={{ fontSize: 12.5, marginBottom: 16, color: 'var(--text-3)' }}>Pick a template below, or set up sections on the left — then Generate.</div>
                    <div style={{ display: 'grid', gap: 8, textAlign: 'left' }}>{[{ k: 'mcq', t: 'MCQ quiz', d: '10 multiple-choice questions', secs: [{ title: 'Multiple choice', type: 'mcq', count: 10, marks: 1 }] }, { k: 'unit', t: 'Unit test', d: '3 sections · MCQ, short, long', secs: [{ title: 'Part A · Multiple choice', type: 'mcq', count: 5, marks: 1 }, { title: 'Part B · Short answer', type: 'short', count: 4, marks: 2 }, { title: 'Part C · Long answer', type: 'long', count: 2, marks: 5 }] }, { k: 'mixed', t: 'Mixed practice', d: 'MCQ, fill, true/false, short', secs: [{ title: 'Part A · Multiple choice', type: 'mcq', count: 5, marks: 1 }, { title: 'Part B · Fill in the blanks', type: 'fill', count: 4, marks: 1 }, { title: 'Part C · True / false', type: 'tf', count: 4, marks: 1 }, { title: 'Part D · Short answer', type: 'short', count: 3, marks: 2 }] }].map((tpl) => (<button key={tpl.k} type="button" onClick={() => applyStarter(tpl.secs)} data-testid={'starter-' + tpl.k} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', textAlign: 'left', background: 'var(--glass-1)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '10px 12px', cursor: 'pointer', color: 'var(--text)', fontFamily: 'inherit' }}><span aria-hidden="true" style={{ fontSize: 15, color: 'var(--violet-2)' }}>{'▦'}</span><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: 'block', fontSize: 13, color: 'var(--text)' }}>{tpl.t}</span><span style={{ display: 'block', fontSize: 11, color: 'var(--text-4)' }}>{tpl.d}</span></span><span aria-hidden="true" style={{ fontSize: 12, color: 'var(--text-3)' }}>{'→'}</span></button>))}</div>
                  </div>
                )}
              </div>
            ) : (
              <div id="result-top">
                ...
                {editAns && (
                  <div className="no-print glass" style={{ padding: '16px 18px', borderRadius: 'var(--r-lg)', maxWidth: 820, margin: '0 auto 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}><div className="eyebrow" style={{ margin: 0 }}>Edit questions &amp; answers</div><button type="button" onClick={addHotspot} data-testid="add-hotspot" className="btn btn-glass btn-sm" style={{ fontSize: 11, padding: '2px 9px' }} title="Add an auto-graded image-click (hotspot) question">+ Image-click question</button></div>
                    ...
                    {(() => { let n = 0; return paper.sections.flatMap((sec) => sec.questions.map((q, li) => { const gi = n++; return (
                      <div key={gi} style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8, fontSize: 13 }}>
                        ...
                        <EditAnswerControl q={q} gi={gi} onPatch={editPatch} /><label className="btn btn-glass btn-sm" title={q.image ? 'Change image' : 'Add an image / diagram'} data-testid={'qimg-add-' + gi} style={{ padding: '2px 8px', cursor: 'pointer', fontSize: 12 }}>{q.image ? '\uD83D\uDDBC \u2713' : '\uD83D\uDDBC'}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => { onQImageFile(e.target.files && e.target.files[0], gi); e.target.value = ''; }} style={{ display: 'none' }} data-testid={'qimg-input-' + gi} /></label>{q.image ? <button type="button" onClick={() => editPatch(gi, { image: '' })} data-testid={'qimg-clear-' + gi} className="btn btn-glass btn-sm" style={{ padding: '2px 7px', fontSize: 11 }} aria-label="Remove image">✕ img</button> : null}
                        <button type="button" onClick={() => moveQuestion(gi, -1)} disabled={li === 0} className="btn btn-glass btn-sm" style={{ padding: '2px 7px' }} aria-label="Move question up" data-testid={'qup-' + gi}>{'↑'}</button><button type="button" onClick={() => moveQuestion(gi, 1)} disabled={li === sec.questions.length - 1} className="btn btn-glass btn-sm" style={{ padding: '2px 7px' }} aria-label="Move question down" data-testid={'qdown-' + gi}>{'↓'}</button><button type="button" onClick={() => regenQ(gi)} disabled={regenGi != null} title="Replace this question with a fresh AI-generated one (uses a credit)" className="btn btn-glass btn-sm" style={{ padding: '2px 9px' }} data-testid={'regen-' + gi}>{regenGi === gi ? '…' : '↻'}</button>
                      </div>
                    ); })); })()}
                  </div>
                )}
                {view === 'paper' ? (
                  <div id="paper-print" style={{ position: 'relative', background: '#fff', color: '#111', borderRadius: 'var(--r-lg)', padding: '40px 44px', maxWidth: 820, margin: '0 auto', border: '1px solid var(--stroke-2)', boxShadow: '0 10px 40px rgba(0,0,0,0.30)' }}><span className="no-print" data-testid="copy-badge" style={{ position: 'absolute', top: 8, right: 12, fontSize: 9.5, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#999', border: '1px solid #e5e5e5', borderRadius: 999, padding: '1px 8px' }}>{includeKey ? 'Teacher copy' : 'Student copy'}</span>{watermark && (paper.institution || paper.title) ? <div aria-hidden="true" data-testid="watermark" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}><span style={{ transform: 'rotate(-30deg)', whiteSpace: 'nowrap', fontSize: Math.max(22, Math.min(48, Math.round(1050 / Math.max((paper.institution || paper.title || '').slice(0, 44).length, 1)))), fontWeight: 700, letterSpacing: '0.06em', color: 'rgba(0,0,0,0.06)', textTransform: 'uppercase', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>{(paper.institution || paper.title || '').slice(0, 44)}</span></div> : null}
