...
export default function PapersPage() {
  ...
  const [secOpen, setSecOpen] = useState({ lib: true, shares: true, bank: true });
  ...
  useEffect(() => { try { const a = JSON.parse(localStorage.getItem('cwpai_qpg_templates') || '[]'); if (Array.isArray(a)) setTemplates(a); } catch (e) {} }, []);
  ...
  function saveTemplate() { if (typeof window === 'undefined') return; const name = (window.prompt('Save this section setup as a reusable template (this browser). Name:', examStyle || (sections[0] && sections[0].title) || 'My template') || '').trim(); if (!name) return; const tpl = { id: Date.now(), name: name.slice(0, 60), sections: sections.map((s) => ({ ...s })), difficulty, cognitive }; setTemplates((cur) => { const next = [...cur.filter((t) => t.name !== name), tpl].slice(-20); try { localStorage.setItem('cwpai_qpg_templates', JSON.stringify(next)); } catch (e) {} return next; }); }
  function applyTemplate(tpl) { if (!tpl) return; applyStarter(tpl.sections || []); if (tpl.difficulty) setDifficulty(tpl.difficulty); if (tpl.cognitive !== undefined) setCognitive(tpl.cognitive || ''); }
  function delTemplate(id) { setTemplates((cur) => { const next = cur.filter((t) => t.id !== id); try { localStorage.setItem('cwpai_qpg_templates', JSON.stringify(next)); } catch (e) {} return next; }); }
        ...
        <div id="main" className="papers-main" data-mtab={mobileTab} style={{ display: 'flex', flex: 1, minWidth: 0 }}><div className="qpg-tabs no-print" data-testid="mobile-tabs" style={{ gap: 6, padding: '8px 12px', borderBottom: '1px solid var(--stroke-1)', background: 'rgba(5,6,20,0.4)' }}><button type="button" onClick={() => setMobileTab('build')} className={mobileTab === 'build' ? 'btn btn-iris btn-sm' : 'btn btn-glass btn-sm'} data-testid="mtab-build" aria-pressed={mobileTab === 'build'} style={{ flex: 1 }}>Build</button><button type="button" onClick={() => setMobileTab('preview')} className={mobileTab === 'preview' ? 'btn btn-iris btn-sm' : 'btn btn-glass btn-sm'} data-testid="mtab-preview" aria-pressed={mobileTab === 'preview'} style={{ flex: 1 }}>Preview</button></div>
          ...
          <section className="no-print papers-build" style={{ display: buildOpen ? 'block' : 'none', width: asideOpen ? 472 : 660, flexShrink: 0, borderRight: '1px solid var(--stroke-1)', overflowY: 'auto', padding: '18px 20px', background: 'rgba(5,6,20,0.35)' }}>
            ...
            {isFullRO ? (
              ...
              <button type="button" onClick={addSec} disabled={sections.length >= 8} className="btn btn-glass btn-sm" data-testid="add-section" style={{ marginTop: 2 }}>+ Add section</button>{sections.length >= 8 && <span style={{ fontSize: 11, color: 'var(--text-4)', marginLeft: 8 }}>Up to 8 sections</span>}<button type="button" onClick={saveTemplate} className="btn btn-glass btn-sm" data-testid="save-template" style={{ marginTop: 2, marginLeft: 6 }} title="Save these sections as a reusable template (this browser)">★ Save as template</button>{templates.length > 0 && <div data-testid="my-templates" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>{templates.map((tpl) => <span key={tpl.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11.5, background: 'var(--glass-1)', border: '1px solid var(--stroke-2)', borderRadius: 'var(--r)', padding: '3px 4px 3px 9px' }}><button type="button" onClick={() => applyTemplate(tpl)} data-testid="tpl-apply" title="Apply this template" style={{ border: 'none', background: 'transparent', color: 'var(--text-2)', cursor: 'pointer', fontSize: 11.5, padding: 0 }}>{tpl.name}</button><button type="button" onClick={() => delTemplate(tpl.id)} aria-label={'Delete template ' + tpl.name} data-testid="tpl-del" style={{ border: 'none', background: 'transparent', color: 'var(--text-4)', cursor: 'pointer', fontSize: 12, lineHeight: 1, padding: '0 2px' }}>{'✕'}</button></span>)}</div>}
            </>)}
            </>)}
