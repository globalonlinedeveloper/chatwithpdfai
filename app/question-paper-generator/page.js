...
export default function PapersPage() {
  ...
  useEffect(() => { try { const u = new URLSearchParams(window.location.search); if (u.get('paper') || u.get('doc')) return; const d = JSON.parse(localStorage.getItem('cwpai_qpg_draft') || 'null'); if (d && Array.isArray(d.sections) && d.sections.length) { if (typeof d.topic === 'string') setTopic(d.topic); setSections(d.sections); if (d.institution) setInstitution(d.institution); if (d.instructions) setInstructions(d.instructions); if (d.difficulty) setDifficulty(d.difficulty); if (d.sets) setSets(d.sets); if (d.examStyle) setExamStyle(d.examStyle); } } catch (e) {} }, []);
  useEffect(() => { try { localStorage.setItem('cwpai_qpg_draft', JSON.stringify({ topic, sections, institution, instructions, difficulty, sets, examStyle })); } catch (e) {} }, [topic, sections, institution, instructions, difficulty, sets, examStyle]);
  useEffect(() => {
  ...
  async function generate() {
    ...
    try {
      ...
      setPaper(master); setUsed(j.credits); setCurSet(0); setUndoQ(null);
