              ...
              {sections.map((s, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 7, padding: '8px 9px', background: 'var(--glass-1)', border: '1px solid var(--stroke-1)', borderRadius: 'var(--r)' }} data-testid="section-row">
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <label style={{ fontSize: 11.5, color: 'var(--text-3)' }}>Marks <input type="number" min={1} max={20} step={0.5} value={s.marks} onChange={(e) => setSec(i, { marks: clampHalf(e.target.value, 1, 20) })} aria-label="Marks per question" className="qpg-num" style={{ ...ctrl, width: 58 }} /></label><label style={{ fontSize: 11.5, color: 'var(--text-3)' }} title="Answer any N of these questions (0 = all compulsory)">Any <input type="number" min={0} max={Number(s.count) || 30} value={s.choose || 0} onChange={(e) => setSec(i, { choose: clampInt(e.target.value, 0, Number(s.count) || 30) })} aria-label="Answer any N (0 = all compulsory)" className="qpg-num" data-testid="sec-choose" style={{ ...ctrl, width: 46 }} /></label><span data-testid="sec-marks" title="Section total = questions × marks" style={{ fontSize: 11, color: 'var(--text-3)', whiteSpace: 'nowrap', minWidth: 34, textAlign: 'right' }}>= {(((Number(s.choose) || 0) > 0 && (Number(s.choose) || 0) < (Number(s.count) || 0)) ? (Number(s.choose) || 0) : (Number(s.count) || 0)) * (Number(s.marks) || 0)}</span>
                    <button type="button" onClick={() => moveSec(i, -1)} disabled={i === 0} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Move section up" data-testid={'secup-' + i}>{'↑'}</button><button type="button" onClick={() => moveSec(i, 1)} disabled={i === sections.length - 1} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Move section down" data-testid={'secdown-' + i}>{'↓'}</button><button type="button" onClick={() => dupSec(i)} disabled={sections.length >= 8} className="btn btn-glass btn-sm" style={{ padding: '5px 8px' }} aria-label="Duplicate section" data-testid={'secdup-' + i} title="Duplicate this section">{'⧉'}</button><button type="button" onClick={() => delSec(i)} className="btn btn-glass btn-sm" style={{ padding: '5px 9px' }} aria-label="Remove section">{'✕'}</button>
                  </div>
