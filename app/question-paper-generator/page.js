          ...
          <section className="papers-preview" style={{ flex: 1, minWidth: 0, overflowY: 'auto', padding: '18px 20px', background: 'rgba(5,6,20,0.5)' }}>
            ...
            {!paper ? (
              ...
              <div id="result-top">
                ...
                {view === 'paper' ? (
                  <div id="paper-print" style={{ position: 'relative', background: '#fff', color: '#111', borderRadius: 'var(--r-lg)', padding: '40px 44px', maxWidth: 820, margin: '0 auto', border: '1px solid var(--stroke-2)', boxShadow: '0 10px 40px rgba(0,0,0,0.30)' }}><span className="no-print" data-testid="copy-badge" style={{ position: 'absolute', top: 8, right: 12, fontSize: 9.5, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#999', border: '1px solid #e5e5e5', borderRadius: 999, padding: '1px 8px' }}>{includeKey ? 'Teacher copy' : 'Student copy'}</span>{watermark && (paper.institution || paper.title) ? <div aria-hidden="true" data-testid="watermark" style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}><div style={{ position: 'absolute', top: '-25%', left: '-25%', width: '150%', height: '150%', transform: 'rotate(-30deg)', transformOrigin: 'center', display: 'flex', flexWrap: 'wrap', alignContent: 'flex-start', justifyContent: 'center', gap: '48px 70px' }}>{Array.from({ length: 280 }).map((_, wmI) => <span key={wmI} style={{ fontSize: 21, fontWeight: 700, letterSpacing: '0.12em', color: 'rgba(0,0,0,0.05)', whiteSpace: 'nowrap', textTransform: 'uppercase', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>{(paper.institution || paper.title || '').slice(0, 40)}</span>)}</div></div> : null}
