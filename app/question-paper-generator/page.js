...
export default function PapersPage() {
  ...
  function saveTemplate() { if (typeof window === 'undefined') return; const name = (window.prompt('Save the current build setup as a reusable template. Name:', (bpKey !== 'custom' && examStyle) || topic || (sections[0] && sections[0].title) || 'My template') || '').trim(); if (!name) return; const config = { topic, bpTopic, examStyle, bpKey, sections: sections.map((x) => ({ ...x })), difficulty, cognitive, language, sets, layout, negMark, watermark, includeKey, verify, institution, instructions }; fetch('/api/papers/templates', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.slice(0, 120), config }) }).then((r) => r.json().catch(() => ({}))).then((j) => { if (j && j.ok) { setSavedMsg('Template saved'); setSecOpen((x) => ({ ...x, templates: true })); loadTemplates(); setTimeout(() => setSavedMsg(''), 1800); } else setSavedMsg((j && j.error) || 'Could not save template'); }).catch(() => {}); }
