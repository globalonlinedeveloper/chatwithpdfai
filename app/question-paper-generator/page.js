...
export default function PapersPage() {
  ...
  async function openPaper(id) { try { const r = await fetch('/api/papers/library?id=' + id); const j = await r.json().catch(() => ({})); if (r.ok && j.paper) { setPaper(j.paper); setLogo(j.paper.logo || ''); setCurSet(0); setLayout(j.paper.layout || 'official'); setView('paper'); setChecked(false); setAnswers({}); setUsed(null); setTimeout(() => { const el = document.getElementById('result-top'); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 60); } } catch (e) {} }
  ...
  async function generate() {
    ...
    try {
      ...
      setPaper({ ...master, logo }); setUsed(j.credits); setCurSet(0); setUndoQ(null);
