...
export function EditAnswerControl({ q, gi, onPatch }) {
  ...
  if (q.type === 'mcq' || q.type === 'code' || q.type === 'assertion') return <span style={{ display: 'inline-flex', flexDirection: 'column', gap: 4, minWidth: 220 }}>{q.options.map((o, oi) => <label key={oi} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5 }}><input type="radio" name={'ans' + gi} checked={q.answer === oi} onChange={() => onPatch(gi, { answer: oi })} aria-label={'Mark option ' + LETTER(oi) + ' correct'} /><span style={{ color: 'var(--text-3)' }}>({LETTER(oi)})</span><input value={String(o)} onChange={(e) => onPatch(gi, { options: q.options.map((x, j) => j === oi ? e.target.value : x) })} aria-label={'Option ' + LETTER(oi) + ' text'} data-testid={'opt-' + gi + '-' + oi} className="input" style={{ flex: 1, minWidth: 140, fontSize: 12.5, padding: '4px 8px' }} /></label>)}</span>;
