...
function TakeInput({ q, ua, locked, onA, optPerm }) {
  const btn = (label, active, onClick, kind) => <button type="button" role={kind || undefined} onClick={onClick} disabled={locked} {...((kind === 'radio' || kind === 'checkbox') ? { 'aria-checked': !!active } : { 'aria-pressed': !!active })} style={{ textAlign: 'left', padding: '9px 12px', borderRadius: 'var(--r)', background: active ? 'var(--glass-2)' : 'var(--glass-1)', border: '1px solid ' + (active ? 'var(--violet)' : 'var(--stroke-2)'), color: 'var(--text)', fontSize: 13.5, cursor: locked ? 'default' : 'pointer', width: '100%' }}>{label}</button>;
  if (q.type === 'mcq' || q.type === 'code' || q.type === 'assertion') { const order = optPerm || q.options.map((_, i) => i); return <div role="radiogroup" aria-label="Answer options" style={{ display: 'grid', gap: 6, marginTop: 8 }}>{order.map((oi, di) => <div key={oi}>{btn(<span>({LETTER(di)}) {q.options[oi]}</span>, ua === oi, () => onA(oi), 'radio')}</div>)}</div>; }
  if (q.type === 'tf') return <div role="radiogroup" aria-label="True or false" style={{ display: 'flex', gap: 8, marginTop: 8 }}>{[true, false].map((v) => <div key={String(v)} style={{ flex: 1 }}>{btn(v ? 'True' : 'False', ua === v, () => onA(v), 'radio')}</div>)}</div>;
  if (q.type === 'multi') { const order = optPerm || q.options.map((_, i) => i); const arr = Array.isArray(ua) ? ua : []; return <div role="group" aria-label="Select all that apply" style={{ display: 'grid', gap: 6, marginTop: 8 }}>{order.map((oi, di) => { const sel = arr.includes(oi); return <div key={oi}>{btn(<span>{sel ? '☑' : '☐'} ({LETTER(di)}) {q.options[oi]}</span>, sel, () => onA(sel ? arr.filter((x) => x !== oi) : [...arr, oi]), 'checkbox')}</div>; })}</div>; }
  ...
  if (q.type === 'hotspot') { const pt = Array.isArray(ua) ? ua : null; return q.image ? <div style={{ position: 'relative', display: 'inline-block', marginTop: 8 }}><img src={q.image} alt="" onClick={(e) => { if (locked) return; const r = e.currentTarget.getBoundingClientRect(); onA([Math.round(((e.clientX - r.left) / r.width) * 1000) / 1000, Math.round(((e.clientY - r.top) / r.height) * 1000) / 1000]); }} data-testid="hotspot-img" style={{ maxWidth: '100%', maxHeight: 320, display: 'block', cursor: locked ? 'default' : 'crosshair', borderRadius: 4 }} />{pt ? <span style={{ position: 'absolute', left: (pt[0] * 100) + '%', top: (pt[1] * 100) + '%', transform: 'translate(-50%, -50%)', color: '#e5484d', fontWeight: 700, fontSize: 20, textShadow: '0 0 3px #fff', pointerEvents: 'none' }}>✕</span> : null}</div> : <div style={{ color: 'var(--text-4)', fontSize: 13 }}>(image missing)</div>; }
          ...
          {qOrder.map((gi, di) => { const q = flat[gi]; const r = resById[gi]; return (
            <div key={gi} style={{ marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--stroke-1)' }}>
              ...
              <div style={{ marginLeft: 22 }}>
                {q.image && q.type !== 'hotspot' ? <img src={q.image} alt="" style={{ display: 'block', maxWidth: '100%', maxHeight: 240, margin: '4px 0 8px', borderRadius: 4 }} /> : null}
