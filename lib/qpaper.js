...
export function cleanTitle(raw, examStyle, topic, sourceName, sectionTitles) {
  const t = String(raw == null ? '' : raw).trim().slice(0, 140);
  const tl = t.toLowerCase();
  const secHit = Array.isArray(sectionTitles) && sectionTitles.some((s) => { const v = String(s == null ? '' : s).trim().toLowerCase(); return v && v === tl; });
